    ...
    do{
        ...
        if(aux!=NULL){
            ...
            while((strcmp(sorteada->palavra, pSorteio) != 0)){
                ...
                printf(" Dica: %s", sorteada->assunto);
                printf("\nDigite uma letra:\n");
                scanf("%c", &letra);
                for(int i=0; i <=tamPlv; i++){
            ...
            system("cls");
            if(perdeu == 1){
                printf("Voce perdeu! :( Numero de tentativas: %d\n", tentativas/2);
            }else{
                printf("Voce acertou a palavra! Numero de tentativas: %d \n", tentativas/2);
            }

            system("pause");
            aux = NULL;
        }else{
            printf("Acabou o jogo! Pressione a tecla f para sair\n\n");
        }
    }while(getchar()!='f');
}
    ...
    do{
        ...
        scanf("%d", &escolha);
            ...
            case 1:
                printf("Digite seu nome: ");
                scanf("%s", nome);
                printf("Seu nome foi gravado com sucesso, %s!\n", nome);
                system("pause");
            break;
